...
affected, the root cause in one sentence, and the proposed fix or next step.
One paragraph. No bullet points, no code blocks, no headings inside it. This
TLDR is what the user sees inline in chat — everything above it is filed as
the deep report and only opened on demand, so the TLDR must stand alone.
