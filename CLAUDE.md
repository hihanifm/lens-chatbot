...
**HTTP egress logging** (`httpEgressLogging` feature flag, default on): `src/services/httpEgressLogger.ts` installs a `globalThis.fetch` interceptor at startup and uses `AsyncLocalStorage` to correlate outbound calls with the active session's workspace. When enabled, every fetch to the configured LLM `baseUrl` host (or known LLM hosts `api.openai.com` / `api.anthropic.com`) is dumped to `<workspacePath>/agent_notes/http-egress-<ts>-<seq>.json` with full request body, response body (clone-tee — works for SSE streams), redacted `Authorization` / `x-api-key` headers, and `durationMs`. Compare against `llm-request-*.json` (from the `beforeModel` hook) to verify the composed system prompt and user message reach the wire intact.
